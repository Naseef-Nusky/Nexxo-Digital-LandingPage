export default function PackageCard({ pkg, featured = false }) {
  return (
    <article
      className={`group relative flex h-full flex-col overflow-hidden rounded-2xl border p-3.5 transition duration-300 sm:p-5 md:p-6 ${
        featured
    ...
    >
      ...
      <div className="mt-3 border-t border-white/10 pt-3 sm:mt-5 sm:pt-4">
        <p className="text-xs font-semibold tracking-[0.2em] text-white/45 uppercase sm:text-sm">
          Includes
        </p>
        <ul className="mt-2 flex-1 space-y-2 text-sm text-white/80 sm:mt-3 sm:space-y-2.5 sm:text-base">
      ...
      <a
        href="#quote"
        className={`mt-4 inline-flex items-center justify-center rounded-full px-5 py-3 text-sm font-bold transition sm:mt-6 sm:px-6 sm:py-3.5 sm:text-base ${
          featured
            ? 'btn-brand'
